...
class RegistroM
{
    ...
    {
        $count = 0;
        $query  = 'SELECT COUNT(REGISTRO.COD_REGISTRO) AS TOTAL FROM REGISTRO';
        $query .= ' WHERE ';
        $query .= " DNI = '" . $t->getDni() . "'";
...
public static function readM($parameters)
{
    ...
    $query .= 'SELECT COUNT(REGISTRO.COD_REGISTRO) AS TOTAL FROM REGISTRO';
    $query .= ' WHERE ';
    $query .= " (LOWER(DNI) LIKE :DNI OR LOWER(NOMBRES) LIKE :NOMBRES ";
    $query .= " OR LOWER(APELLIDOS) LIKE :APELLIDOS) ";
   // $query .= $parameters['gender'];
    try {
        ...
        $stmt->bindParam(':NOMBRES', $parameters['filter'], PDO::PARAM_STR);
        $stmt->bindParam(':APELLIDOS', $parameters['filter'], PDO::PARAM_STR);
